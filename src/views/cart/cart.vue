        ...
        <div class="extr">
          <div class="address">
            ...
            <ul>
              ...
              <!-- 收货人地址 -->
              <li @click="selectAddr(item)"  v-for="item in addrInfo" :key="item.id"  :class="{'addressActive': addressActive==item.id }">
          ...
          <div class="pay">
            ...
            <p class="payWrap">
              <!-- 支付方式 -->
              <img v-for="item in payWarapList" :key="item.id" :src="item.img" :class="{'payWarapActive': payWarpActive==item.id}" @click="selectPay(item.id)">

            </p>
          </div>
        </div>
        <textarea v-model="post_script" placeholder="请输入留言" style="margin-top: 10px; height:50px;width: 100%;"></textarea>
        <p class="sumup"><a @click="balanceCount" class="btn">去结算</a></p>
      </div>
    </div>
  </div>
...
<script>
import {getShopCarts, deleteShopCarts, updateShopCarts, delAllShopCart, getAddress, createOrder} from '../../api/api'
...
export default {
  ...
  methods: {
    deleteGoods (index, id) { // 移出购物车
      if (confirm('您确定把该商品移出购物车嘛？')) {
        deleteShopCarts(id).then(response => {
          // console.log(response.data)
    ...
    reduceCartNum (index, id) { // 删除数量
      if (this.goods_list[index].nums <= 1) { // 表示删除商品
        ...
        updateShopCarts(id, {
          ...
          this.$store.dispatch('setSopList')
          this.setTotalPrice()
        }).catch(error => {
          console.log(error)
        })
    ...
    selectPay (id) {
      this.payWarpActive = id
      console.log('支付方式：' + this.payWarpActive)
    },
    balanceCount () { // 结算
      if (this.address === '') {
        alert('请选择收货地址')
