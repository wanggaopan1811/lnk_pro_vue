        ...
        <div class="hd_cart" id="ECS_CARTINFO" @mouseover="overShopCar" @mouseout="outShopCar">
          ...
          <div class="list" v-show="showShopCar">
            ...
            <div class="count">共<span class="red" id="hd_cart_count">{{goodsList.goods_list.length}}</span>件商品哦~
              <p>
                总价:<span class="red"><em id="hd_cart_total">{{goodsList.goods_list.totalPrice}}</em></span>
...
<script>
import { getCategory1, getHotSearch, deleteShopCarts } from '../../api/api'
import { mapGetters } from 'vuex'
export default {
  data () {
    return {
      isShowVip: false, // 会员中心子菜单显示控制
      showAllmenu: false, // 全部端口分类显示控制
      showShopCar: false, // 购物车的显示控制
      showChildrenMenu: -1, // 2\ 3级子菜单显示控制
      allMenuLabel: [], // 各级菜单列表
      hotSearch: [], // 热词
      searchWord: '' // 搜索词
    }
  ...
  methods: {
    ...
    getMenu () { // 获取菜单 前端展示
      getCategory1({
        params: {}
      }).then(response => { // then成功情况
        console.log(response, 'listlist')
        // this.allMenuLabel = response.data
      }).catch(error => { // catch失败情况
        console.log(error)
      })
    },
    getHotSearch () {
      getHotSearch().then(reseponse => {
        console.log(reseponse.data) // 打印出来
        this.hotSearch = reseponse.data // 赋值给 上边的 hotSearch
      }).catch(error => {
        console.log(error)
      })
    },
...
.hd_main .intro li {
    ...
    width:150px;
    height:46px;
    background:url(../../../images/head/webintro-ico.png) 0 -9999px no-repeat;
    overflow:hidden
}
.hd_main .intro li a {
    display:block;
